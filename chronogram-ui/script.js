...
function newOnMouseMove(inEl, outEl, screenshots, screenshotCount) {
  return function(ev) {
    ...
    if (inElLocalTouchedX < 0) {
      inElLocalTouchedX = 0
    }
    if (inElLocalTouchedY < 0) {
      inElLocalTouchedY = 0
    }
    if (inElLocalTouchedX > inEl.offsetWidth) {
      inElLocalTouchedX = inEl.offsetWidth
    }
    if (inElLocalTouchedY > inEl.offsetHeight) {
      inElLocalTouchedY = inEl.offsetHeight
    }
    if (percentX < 0) {
      percentX = 0
    }
    ...
    if (marker) {
      ...
      marker.style.left =
        'calc( ' + percentX * 100 + '% - ' + marker.offsetWidth + 'px )'
    }
  }
}
...
function makeSlide(inEl, outEl, screenshots, screenshotCount) {
  if (inEl) {
    inEl.addEventListener(
      'mousemove',
      newOnMouseMove(inEl, outEl, screenshots, screenshotCount)
    )
  }
}
...
window.addEventListener('DOMContentLoaded', ev => {
  ...
  for (let el of els) {
    let outEl = el.parentElement.parentElement.getElementsByClassName(
      'screenshot'
    )[0]
    console.log(outEl)
    makeSlide(
